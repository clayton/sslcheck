...
module SSLCheck
  describe Checker do
    ...
    it 'should not be failed by default' do
      sut = Checker.new(FakeClient.new)
    ...
    describe 'checking' do
      it 'should tell the client to get the certs' do
        ...
        @sut = Checker.new(fake_client, FakeValidator.new)
      ...
      it 'should expose the certificates that were found' do
        @sut = Checker.new(FakeClient.new(FakeClientResponse.new(@peer_cert, @ca_bundle)), FakeValidator.new)
        @sut.check('www.example.com')
      ...
      it 'should expose the hostname parsed from the URL' do
        @sut = Checker.new(FakeClient.new(FakeClientResponse.new(@peer_cert, @ca_bundle, "www.example.com")), FakeValidator.new)
        @sut.check('www.example.com')
      ...
      context "when there is an error checking the certificate" do
        it 'should not be valid' do
          ...
          @sut  = Checker.new(FakeClient.new(nil, [error]), FakeValidator.new)
          @sut.check('www.example.com')
          expect(@sut.valid?).to_not be
        end
        context "when the URI is malformed" do
          it 'should add an error to the error list' do
            ...
            @sut  = Checker.new(FakeClient.new(nil, [error]), validator)
          ...
          it 'should not try to validate the certificate' do
            error = SSLCheck::Errors::GenericError.new({:name => :invalid_uri, :message => "Invalid URI"})
            @sut  = Checker.new(FakeClient.new(nil, [error]))
            @sut.check('www.example.com')
          end
        end
    ...
    describe 'validation' do
      it 'should tell the validator to validate the peer certificate' do
        validator = FakeValidator.new
        @sut = Checker.new(FakeClient.new(FakeClientResponse.new(@peer_cert, @ca_bundle)), validator)
      ...
      context "when the certificate is invalid" do
        it 'should bubble up any errors found during validation' do
          @sut = Checker.new(FakeClient.new(FakeClientResponse.new(@peer_cert, @ca_bundle)), FakeValidator.new(false, [SSLCheck::Errors::GenericError.new({:name => "generic error", :message => "generic error"})]))
          @sut.check('www.example.com')
